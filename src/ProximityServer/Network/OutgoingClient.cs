...
{
  /// <summary>
  /// Outgoing client class represents a TCP client the proximity server, in the role of a client, used to connect to other proximity servers.
  /// </summary>
  public class OutgoingClient : ClientBase, IDisposable
  {
      ...
      base(RemoteEndPoint, UseTls)
    {
      ...
      log = new Logger("ProximityServer.Network.OutgoingClient", logPrefix);

      log.Trace("()");

      messageBuilder = new ProxMessageBuilder(0, new List<SemVer>() { SemVer.V100 }, Config.Configuration.Keys);
