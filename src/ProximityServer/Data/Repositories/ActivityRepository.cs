...
{
  /// <summary>
  /// Generic repository for activities, which is the base for PrimaryIdentityReposity for primary identities of this proximity server
  /// and NeighborActivityRepository for identities managed by this proximity server's neighbors.
  /// </summary>
  public class ActivityRepository<T> : GenericRepository<Context, T> where T : ActivityBase
  {
    ...
    public async Task<List<T>> ProfileSearch(uint ResultOffset, uint MaxResults, string TypeFilter, string NameFilter, GpsLocation LocationFilter, uint Radius)
    {
      ...
      // If we are querying HostedActivityRepository we need to make sure only active identities are counted in.
      bool homeRepo = this is HostedActivityRepository;
      if (homeRepo)
